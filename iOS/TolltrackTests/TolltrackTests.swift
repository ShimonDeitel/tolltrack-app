import XCTest
@testable import Tolltrack

@MainActor
final class TolltrackTests: XCTestCase {
    var store: TolltrackStore!

    override func setUp() async throws {
        store = TolltrackStore()
    }
    ...
    func testFreeLimitBlocksAddition() {
        store.isPro = false
        for i in 0..<(TolltrackStore.freeLimit + 5) {
            store.add(title: "Filler \(i)", amount: 1.0)
        }
        XCTAssertEqual(store.entries.count, TolltrackStore.freeLimit)
    }
    ...
    func testCanAddMoreWhenPro() {
        ...
        XCTAssertGreaterThan(store.entries.count, TolltrackStore.freeLimit)
    }
    ...
    func testFreeLimitAboveSeedCount() {
        XCTAssertGreaterThan(TolltrackStore.freeLimit, 3)
    }
}
